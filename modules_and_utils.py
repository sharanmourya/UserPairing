...
# #fast version
def decode_clique_final_speed(data, weights, probabilities, draw=False, weight_factor = 0.35, clique_number_bounds = None ,fig = None, device = 'cpu', beam = 1):
    ...
    for graph in range(num_graphs):
        ...
        for node in range(beam_width):
            ...
            neighborhood_probs =  graph_probs[target_neighborhood].squeeze(-1)
#             neighborhood_probs =  torch.unique(weights[nlr_graph == sorted_inds[node]]) * graph_probs[target_neighborhood].squeeze(-1)
#             neighborhood_probs =  torch.unique(weights[nlr_graph == sorted_inds[node]].squeeze(-1)) * graph_probs[target_neighborhood].squeeze(-1)
            ...
            for node_2 in sorted_target_neighborhood:
                ...
                elif current_set_max_edges == (clique_number_bounds*(clique_number_bounds-1))/2:
                    break
